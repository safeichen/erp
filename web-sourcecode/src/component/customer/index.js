...
class Customer extends Component {
    ...
    listCustomer = () => {
        ...
        let result = this.props.listCustomer(this.state.queryCondition, pagination.current || 1, pagination.pageSize || 20);
        if(!result) {
            return;
        }
        ...
        this.setState({
            pagination,
            customers: result.vos || []
        });
    }

    render() {
        // FLOOR_TYPE.filter(t => t.pageType===pageType).map((item, index) => {flooTypeGroup.push(<Radio value={item.key}>{item.value}</Radio>)});
        // PLATFORM_TYPE.map((item, index) => {platformGroup.push({ label: item.value, value: item.key })});
        return (
            <div>
                ...
                <div className="query_box">
                    ...
                    <Table className="v-table" locale={{ emptyText: '暂无数据' }} pagination={this.state.pagination}
                        dataSource={this.state.customers} onChange={this.onChangePage}>
...
export default connect(erpState, erpAction)(Customer);
